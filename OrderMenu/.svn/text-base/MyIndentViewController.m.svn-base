...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MyindentCustomCell *cell = (MyindentCustomCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    
    if (cell == nil)
    {
        //记载xib相当于创建了xib当中的内容，返回的数组里面包含了xib当中的对象
        // NSLog(@"新创建的cell  %d",indexPath.row);
        NSArray *array = [[NSBundle mainBundle] loadNibNamed:@"MyindentCustomCell" owner:nil options:nil];
		
        for (NSObject *object in array)
        {
            //判断数组中的对象是不是CustomCell 类型的
            if([object isKindOfClass:[MyindentCustomCell class]])
            {
                //如果是，赋给cell指针
                cell = (MyindentCustomCell *)object;
                //找到之后不再寻找
                break;
            }
        }
...
{
    MyindentDetailViewController *myindentVC=[[MyindentDetailViewController alloc] init];
