...
- (id)initWithViewControllers:(NSArray *)vcs imageArray:(NSArray *)arr
{
	...
	if (self != nil)
	{
		...
		_transitionView.backgroundColor =  [UIColor groupTableViewBackgroundColor];
         ...
         {
           		_tabBar = [[LeveyTabBar alloc] initWithFrame:CGRectMake(0, _containerView.frame.size.height, 320.0f, kTabBarHeight) buttonImages:arr];
         }
        else
        {
        		_tabBar = [[LeveyTabBar alloc] initWithFrame:CGRectMake(0, _containerView.frame.size.height, 320.0f, kTabBarHeight) buttonImages:arr];
...
//        _tabBar=[[LeveyTabBar alloc] initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height-kTabBarHeight, 320.0, kTabBarHeight) buttonImages:arr];
...
- (void)hidesTabBar:(BOOL)yesOrNO animated:(BOOL)animated
{
	...
	if (yesOrNO == YES)
	{
		if (self.tabBar.frame.origin.y+p == self.view.frame.size.height)
		{
	...
	{
		if (self.tabBar.frame.origin.y+p == self.view.frame.size.height - kTabBarHeight)
		{
			return;
		}
        if (self.tabBar.frame.origin.y+p > self.view.frame.size.height)
	...
	if (animated == YES)
	{
		...
		{
            NSLog(@"ios7 = %f",self.tabBar.frame.origin.y - kTabBarHeight);
			self.tabBar.frame = CGRectMake(self.tabBar.frame.origin.x, self.tabBar.frame.origin.y - kTabBarHeight, self.tabBar.frame.size.width, self.tabBar.frame.size.height);
		}
